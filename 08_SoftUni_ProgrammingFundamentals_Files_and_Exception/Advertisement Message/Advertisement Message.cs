...
namespace Advertisement_Message
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                lines[i] = phrases[random.Next(0, phrases.Length)];
                lines[i] = lines[i] + " ";
                lines[i] = lines[i] + events[random.Next(0, events.Length)];
                lines[i] = lines[i] + " ";
                lines[i] = lines[i] + names[random.Next(0, names.Length)];
                lines[i] = lines[i] + " - ";
                lines[i] = lines[i] + cities[random.Next(0, cities.Length)];
